...
class Device(object):
    ...
    这个类描述了一个已连接的Android设备，提供了完整的设备控制和管理功能
    包括：设备连接、应用管理、事件发送、截图、状态监控等
    
    主要功能：
    - 设备连接与适配器管理（ADB、Telnet、Minicap等）
    - 应用安装、卸载、启动、停止
    - 屏幕操作（触摸、滑动、按键）
    - 设备状态获取（当前活动、服务、视图等）
    - 通信功能模拟（电话、短信）
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None,
        ...
        :param device_serial: 设备序列号，如果为None则自动检测第一个可用设备
        :param is_emulator: 是否为模拟器，True表示模拟器，False表示真机
        :param output_dir: 输出目录路径，用于存储截图、日志等文件
        :param cv_mode: 是否使用计算机视觉模式进行界面分析
        :param grant_perm: 是否自动授予应用权限（Android 6.0+）
        :param telnet_auth_token: Telnet控制台认证令牌
        :param enable_accessibility_hard: 是否启用硬件辅助功能
        :param humanoid: 人形模式设置，用于模拟人类操作行为
        :param ignore_ad: 是否忽略广告视图
        :return: 无返回值
        """
        ...
        # 设备适配器初始化
        # 每个适配器负责不同的设备交互功能
        self.adb = ADB(device=self)              # ADB命令适配器：执行基本的ADB命令
        self.telnet = TelnetConsole(device=self, auth_token=telnet_auth_token)  # Telnet控制台：模拟器专用控制
        self.droidbot_app = DroidBotAppConn(device=self)  # DroidBot应用连接：与应用进行通信
        self.minicap = Minicap(device=self)       # Minicap适配器：高性能屏幕截图
        self.logcat = Logcat(device=self)         # 日志捕获适配器：获取系统日志
        self.user_input_monitor = UserInputMonitor(device=self)  # 用户输入监控：记录用户操作
        self.process_monitor = ProcessMonitor(device=self)  # 进程监控：监控应用进程状态
        self.droidbot_ime = DroidBotIme(device=self)  # 输入法适配器：文本输入控制

        # 适配器启用配置字典
        # 键：适配器实例，值：是否启用该适配器
        self.adapters = {
            self.adb: True,              # ADB适配器：默认启用，基础设备操作
            self.telnet: False,          # Telnet适配器：默认禁用，仅用于模拟器
            self.droidbot_app: True,     # DroidBot应用连接：默认启用，应用交互
            self.minicap: True,          # Minicap适配器：默认启用，高性能截图
            self.logcat: True,           # 日志适配器：默认启用，系统日志捕获
            self.user_input_monitor: True,  # 输入监控：默认启用，记录用户操作
            self.process_monitor: True,  # 进程监控：默认启用，监控应用状态
            self.droidbot_ime: True      # 输入法适配器：默认启用，文本输入控制
        }
    ...
    def get_release_version(self):
        ...
        通过ADB命令获取设备的Android系统版本（如：8.0、9.0等）
        使用缓存机制避免重复查询
    ...
    def receive_call(self, phone=DEFAULT_NUM):
        ...
        使用Telnet控制台模拟来电事件（仅适用于模拟器）
        
        :param phone: 来电号码字符串，默认为1234567890
        :return: Telnet命令执行结果
        """
    ...
    def cancel_call(self, phone=DEFAULT_NUM):
        ...
        使用Telnet控制台取消通话（仅适用于模拟器）
        
        :param phone: 电话号码字符串，默认为1234567890
        :return: Telnet命令执行结果
        """
        assert self.telnet is not None
    ...
    def call(self, phone=DEFAULT_NUM):
        ...
        使用Intent启动拨号界面并拨打指定号码
        
        :param phone: 要拨打的电话号码字符串，默认为1234567890
        :return: Intent发送结果
        """
    ...
    def send_sms(self, phone=DEFAULT_NUM, content=DEFAULT_CONTENT):
        ...
        :param phone: 接收者电话号码字符串，默认为1234567890
        :param content: 短信内容字符串，默认为"Hello world!"
        :return: 成功发送返回True
        """
    ...
    def receive_sms(self, phone=DEFAULT_NUM, content=DEFAULT_CONTENT):
        ...
        使用Telnet控制台模拟接收短信（仅适用于模拟器）
        
        :param phone: 发送者电话号码字符串，默认为1234567890
        :param content: 短信内容字符串，默认为"Hello world!"
        :return: Telnet命令执行结果
        """
        assert self.telnet is not None
    ...
    def take_screenshot(self):
        ...
        支持两种截图方式：
        1. 使用Minicap适配器（高性能，jpg格式）
        2. 使用ADB screencap命令（兼容性好，png格式）
        
        :return: 本地截图文件路径，如果输出目录未设置则返回None
        """
        # 旧版截图代码（已注释）
        # image = None
    ...
    def get_current_state(self, action_count=None):
        ...
        :param action_count: 动作计数标签，用于标识状态对应的操作序号
        :return: DeviceState对象，包含完整的设备状态信息
        """
    ...
    def view_long_touch(self, x, y, duration=2000):
        ...
        :param x: 触摸点的X坐标
        :param y: 触摸点的Y坐标
        :param duration: 长按持续时间（毫秒），默认2000ms
        此解决方案由HaMi建议（Stack Overflow用户）
        """
    ...
    def view_drag(self, start_xy, end_xy, duration):
        ...
        实际上使用ADB的input swipe命令实现
        
        :param start_xy: 起始坐标元组 (start_x, start_y)
        :param end_xy: 结束坐标元组 (end_x, end_y)
    ...
    def view_set_text(self, text):
        ...
        """
        if self.droidbot_ime.connected:
            # 使用DroidBot输入法适配器设置文本（模式0：设置）
            self.droidbot_ime.input_text(text=text, mode=0)
        else:
            # ADB的input text命令不支持设置文本，只能追加
