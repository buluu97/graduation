...
## 更新用户配置文件
在更新 Kea2 时，用户的本地配置有时需要更新。（最新的 kea2 版本可能与旧的配置文件不兼容。）

当检测到运行时错误时，Kea2 会检查本地配置文件是否与当前 Kea2 版本兼容。如果不兼容，控制台会打印警告信息。请按照以下说明更新本地配置文件。

1. 备份你的本地配置文件（以防在更新过程中出现意外问题）。
2. 删除项目根目录中 "/configs" 下的所有配置文件。
3. 运行 `kea2 init` 以生成最新的配置文件。
4. 根据你的需要，将旧配置合并到新的配置文件中。
