...
2. **在运行测试时增加命令行参数**  
   ...
   示例运行命令：
   ```
   kea2 run -s "emulator-5554" -p it.feio.android.omninotes.alpha --running-minutes 10 --act-whitelist-file /sdcard/awl.strings --driver-name d unittest discover -p quicktest.py
   ```
...
2. **在运行测试时增加命令行参数**  
   ...
   示例运行命令：
   ```
   kea2 run -s "emulator-5554" -p it.feio.android.omninotes.alpha --running-minutes 10 --act-blacklist-file /sdcard/abl.strings --driver-name d unittest discover -p quicktest.py
   ```
