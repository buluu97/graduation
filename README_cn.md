...
```
> 如果你使用清华、USTC等镜像站点，可能会升级失败，因为这些站点可能尚未同步最新版本。此时可以尝试手动指定版本安装，或直接使用官方PyPI源：`pip install kea2-python -i https://pypi.org/simple`。
...
Kea2支持复用已有的UI测试脚本。我们受到以下理念启发：*已有的UI测试脚本通常覆盖重要的App功能，能达到深层App状态，因此可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此特性允许你使用已有脚本，在脚本中设置断点（即可中断点），并在每个断点后启动Fastbot进行探索。这样你可以先完成登录，再启动Fastbot探索登录后的App状态，帮助Fastbot探索深层状态（Fastbot自身难以完成登录操作）。
...
由于模糊测试后App状态可能不可预测，Kea2提供了`common_teardown`函数，用于清理前后脚本之间的环境。该函数可在`configs/teardown.py`中手动指定。
...
此外，支持通过 `kea2 merge` 合并多次测试的报告（详情见 `kea2 merge -h`）。合并报告对于多次测试非常有用。
...
- [Kea2和PBT技术的常见问题与回答](https://sy8pzmhmun.feishu.cn/wiki/SLGwwqgzIiEuC3kwmV8cSZY0nTg?from=from_copylink)  
