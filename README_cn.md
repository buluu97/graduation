...
kea2 run -p it.feio.android.omninotes.alpha --running-minutes 10 --throttle 500 --max-step 15 --driver-name d unittest discover -p guide_scripts.py 

# 使用 guide_scripts.py 引导，并在每个脚本后启动模糊测试（模糊测试期间检查性质）。
kea2 run -p it.feio.android.omninotes.alpha --running-minutes 10 --throttle 500 --max-step 15 --driver-name d unittest discover -p guide_scripts.py propertytest discover -p quickstart2.py
```
